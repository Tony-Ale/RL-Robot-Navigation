...
class ScenePairBiGRUEncoder(nn.Module):
    """Encode each [robot observation, entity observation] sequence with one shared BiGRU."""
    ...
    def forward(self, sequence: torch.Tensor) -> torch.Tensor:
        if sequence.dim() != 3:
            raise ValueError("Expected sequence shape [batch_entities, time, robot_plus_entity_features].")
...
class JointSceneFusionNet(nn.Module):
    """
    Single-BiGRU social-attention architecture for comparing against SocialContextFusionNet.
    ...
    The model concatenates the robot observation with each entity observation before
    the single shared BiGRU, producing one scene/entity context per entity.
    """
    ...
    def __init__(self, config: Dict):
        ...
        scene_cfg = config["scene_encoding"]
        robot_cfg = config["robot_projection"]
        ...
        pair_input_dim = self.robot_input_dim + self.entity_input_dim
        self.scene_encoder = ScenePairBiGRUEncoder(
            input_dim=pair_input_dim,
            hidden_size=scene_cfg["bigru_hidden_size"],
            num_layers=scene_cfg["bigru_layers"],
            dropout=scene_cfg["dropout"],
        )

        self.robot_projector = _build_mlp(
        ...
        interaction_input_dim = robot_cfg["output_dim"] + self.scene_encoder.output_dim
    ...
    @classmethod
    def from_yaml(cls, path: str) -> "JointSceneFusionNet":
    ...
    def forward(
        ...
        robot_history = self._robot_history(robot_observation)
        robot_single = robot_history[:, -1, :]
        robot_context = self.robot_projector(robot_single)
        scene_context = self._encode_robot_entity_pairs(robot_history, entity_history)

        batch_size, num_entities, _ = scene_context.shape
        if entity_mask is None:
            entity_mask = torch.ones(batch_size, num_entities, dtype=torch.bool, device=scene_context.device)
        else:
            entity_mask = entity_mask.to(device=scene_context.device, dtype=torch.bool)
        ...
        joint_features = torch.cat([repeated_robot, scene_context], dim=-1)
    ...
    def _robot_history(self, robot_observation: torch.Tensor) -> torch.Tensor:
        if robot_observation.dim() == 2:
            robot_observation = robot_observation.unsqueeze(1)
        if robot_observation.dim() == 3:
            if robot_observation.shape[-1] != self.robot_input_dim:
                raise ValueError(f"Expected robot feature dim {self.robot_input_dim}, got {robot_observation.shape[-1]}.")
            return robot_observation
        raise ValueError("Expected robot_observation shape [batch, features] or [batch, time, features].")

    def _encode_robot_entity_pairs(self, robot_history: torch.Tensor, entity_history: torch.Tensor) -> torch.Tensor:
        ...
        if robot_history.shape[0] != batch_size:
            raise ValueError("Robot and entity histories must have the same batch size.")
        if robot_history.shape[1] != time_steps:
            raise ValueError("Robot and entity histories must have the same number of timesteps.")
        if num_entities == 0:
            return entity_history.new_zeros((batch_size, 0, self.scene_encoder.output_dim))

        # Pair each entity frame with the robot frame from the same environment step.
        robot_repeated = robot_history[:, None, :, :].expand(-1, num_entities, -1, -1)
        pair_sequence = torch.cat([robot_repeated, entity_history], dim=-1)
        flat = pair_sequence.reshape(batch_size * num_entities, time_steps, self.robot_input_dim + self.entity_input_dim)
        encoded = self.scene_encoder(flat)
        return encoded.reshape(batch_size, num_entities, self.scene_encoder.output_dim)
    ...
    def _empty_entity_forward(self, robot_context: torch.Tensor, return_attention: Optional[bool]):
        ...
        return {
            ...
            "robot_context": robot_context,
            "scene_context": robot_context.new_zeros((robot_context.shape[0], 0, self.scene_encoder.output_dim)),
            "social_context": social_context,
